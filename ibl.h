...
//		HDROperator::save("brdf_importance.hdr", &now_importance_map);
...

		{
			...
			for (int index = 0; index < width * height; ++index) {
				...
				for (k=0, pos=0; k < int_samples ; ++k) {
					...
					const float u = (float)(ix + qu) / width;
					const float v = (float)(iy + qv) / height;
					...
					Sample s;
					s.dir = out;
					s.pdf = pdf[index] * width * height * (1.0f / (4.0f * kPI));

					ret_samples.push_back(s);
				}
			}
